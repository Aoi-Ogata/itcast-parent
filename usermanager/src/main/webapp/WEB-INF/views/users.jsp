...
<div id="userEdit" class="easyui-window" title="编辑会员" data-options="modal:true,closed:true,iconCls:'icon-save',href:'/user/page/user-edit'" style="width:450px;height:350px;padding:10px;">
        The window content.
</div>
...
var toolbar = [{
    ...
    handler:function(){
        $('#userEdit').window({
            onLoad:function(){
                var userList = $("#userList");
                var sels = userList.datagrid("getSelections");
                sels[0].birthday = new Date(sels[0].birthday).format("yyyy-MM-dd");
                $('#content2').form('load', sels[0]);
            }
        }).window('open');
    }
},{
    ...
    handler:function(){
    	...
    	$.messager.confirm('确认','确定删除ID为 '+ids+' 的会员吗？',function(r){
    	    if (r){
            	$.post("/rest/user",{'ids':ids,'_method':'DELETE'}, function(data,status,xhr){
        			if(xhr.status == 204){
